    ...
    def planning(self, sx, sy, gx, gy):
        ...
        while 1:
            ...
            if current.x == goal_node.x and current.y == goal_node.y:
                ...
                MNFPW = 12 # Maximum number of flights per week
                FCFTE = 2000 # Fixed cost for Twin-engine
                FCFFE = 2500 # Fixed cost for Four-engine
                CFT = 12 # Cost for time 
                F = 20 # Each engine consumes Kg/minute
                usd_bbl = 105.68 # usd/bbl for Asia n Oceania Region


                for add_pass in range(0, 200):
                    passengers = 250 + add_pass # number of passengers
                    Cfuel = usd_bbl/ 119.24047 # cost for fuel
                    m = math.ceil(add_pass/50) # by what value it is increased
                    N = math.ceil(TPPW /passengers) # number of flights

                    if passengers >= 300:
                        Total_cost = (TotalTime * 4 * F * Cfuel + TotalTime * (18 + 2 * m ) + FCFFE)*N
                    else:
                        Total_cost = (TotalTime * 2 * F * Cfuel + TotalTime * (18 + 2 * m ) + FCFTE)*N
